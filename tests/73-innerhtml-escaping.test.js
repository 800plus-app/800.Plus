...
describe('innerHTML — שדות חיצוניים עוברים אסקייפ', () => {
  ...
  test('sEsc — עוטף שמאושר ב-ESCAPERS חייב לברוח גם ממרכאות', () => {
    ...
    /* ⚠ ההגדרה שחולצה נכנסת להודעת הכישלון בכוונה. ב-14.8.2026 הבדיקה הזאת
       נכשלה פעמיים ואז עברה תשע פעמים רצוף בלי שינוי קוד, והמסר לא איפשר
       להבחין בין "sEsc באמת שבורה" לבין "נקראה גרסה ישנה של app.js מהדיסק".
       עם ההגדרה בהודעה, הירייה הבאה עונה על זה בעצמה. */
    const diag = `\nההגדרה שחולצה מ-app.js:\n${m[1]}`;

    assert.ok(!out.includes('"'),
      `sEsc אינה בורחת מ-" — הפלט: ${out}\n` +
      'ההשלכה: `<div title="${sEsc(v)}">` נשבר החוצה מהמאפיין, והשער מאשר אותו בשקט.' + diag);
    assert.ok(!out.includes("'"),
      `sEsc אינה בורחת מ-' — הפלט: ${out}` + diag);
  });
});

...
function canonOf(P, round) {
  ...
  return JSON.stringify(SETS.map(s => {
    ...
    /* ⚠ 16.8.2026 · **וזה קרה שוב, על הגן הבא.** ההערה שמעל נכתבה על גני המשטר
       הצר, והיא צדקה — אבל ארבעת מקדמי התכונה (`aFirst`/`aShare` ושתי הגרסאות
       הצרות) נוספו אחריה ולא נכנסו לצורה הקנונית. **הוכח אמפירית ולא הונח:**
       שינוי של `aShareTight` מ-1.5 ל-1.4 ב-app.js הפיל את טבלת הזהב והשאיר את
       הבדיקה "‏TYPO_PARAMS נושא את טביעת האצבע" **ירוקה** — כלומר השער היה עיוור
       בדיוק לגן שהוא אמור לשמור עליו, פעם שנייה, באותה פונקציה.
       הלקח שנרשם כאן ולא רק תוקן: **צורה קנונית אינה מתחזקת את עצמה.** כל גן חדש
       ב-TYPO_PARAMS חייב להיכנס לכאן באותו קומיט שמוסיף אותו, אחרת הוא נשלח בלי
       שומר. אותו תיקון בדיוק נעשה ב-typo-lab/bank_gate.js:fingerprint.
       ‏null מפורש כשהמקדם חסר · סט ישן שומר על טביעה יציבה. */
    const co = k => (p[k] == null ? null : (round ? r4(p[k]) : p[k]));
...
describe('סובלנות איות · מתג הכיבוי והתלות בלקסיקון', () => {
  ...
  /* ⚠ 28.8.2026 · הזוג `הוֹקִיר רַגְלָיו` / `הִדִּיר רַגְלָיו`. ביקורת דיווחה עליו
     כשתי התנגשויות חדשות בצד הפירוש ("הגיע לעתימ רחחוקות" ו"הגיע לעתמ רחוקות"
     מתקבלות על הראשון ושייכות לשני), ותיארה אותן כשגיאות כתיב שגישרו על ההפרש.
     **המדידה על המאגר הנשלח אומרת אחרת, ולכן היא נעולה כאן ולא רק נכתבה בדוח:**
     לשני הערכים אותה מחרוזת פירוש בדיוק, בכל גרסה של data.js מאז הקומיט הראשון.
     הם נרדפים הדדיים ב-GLOSS_ALT, ולכן כל אחד מהם בקבוצת הבעלים של השני, וזו
     הקבלה שהאפליקציה עושה במתכוון מאז glossAlts.
     ⭐ ומה שמכריע: הפסילה בענף השני כאן חלה גם על המחרוזת **המאויתת נכון**. כלומר
     סובלנות השגיאות אינה מה שפתח את הקבלה, והיא לא מה שיסגור אותה. */
  test('הזוג שחולק פירוש · הקבלה היא פטור הנרדפות, לא סובלנות השגיאות', t => {
    const TWIN = 'הִדִּיר רַגְלָיו', CARD = 'הוֹקִיר רַגְלָיו';
    const REPORTED = ['הגיע לעתימ רחחוקות', 'הגיע לעתמ רחוקות'];
    const CLEAN = 'הגיע לעתים רחוקות';
    const w = Array.from(HE.BANK).find(x => x.term === CARD);
    const o = Array.from(HE.BANK).find(x => x.term === TWIN);
    assert.ok(w && o, `${CARD} או ${TWIN} אינם במאגר · הבדיקה אינה יכולה לרוץ`);
    const segs = Array.from(HE.meaningSegs(w.meaning));
    const seg = HE.norm(CLEAN);
    /* המאגר עתיד להיות מוחלף, ואז ייתכן שהמקטע הזה כבר לא יהיה הפירוש של הכרטיס.
       דילוג מדווח ולא כשל: המקרה חדל להתקיים, וזו אינה רגרסיה. ⛔ אבל הוא **כן**
       מדווח, כדי שמי שמחליף את המאגר יראה שהמקרה שהביקורת דיווחה עליו זז. */
    if (!segs.includes(seg)) {
      t.diagnostic(`⚠ "${CLEAN}" כבר אינו מקטע פירוש של ${CARD} · המאגר הוחלף · הרץ את שער המאגר מחדש על הזוג הזה`);
      t.skip('המקרה שהביקורת דיווחה עליו אינו קיים במאגר הנוכחי');
      return;
    }
    const own = HE.typoOwners(w.meaning, w);
    /* 1 · הפטור עצמו · שני הכיוונים. בלעדיו ההסבר "נרדפים" הוא טענה ולא מדידה. */
    assert.ok(own.has(HE.K(TWIN)), `${TWIN} אינו בקבוצת הבעלים של ${CARD} · הפטור נשבר`);
    assert.ok(HE.typoOwners(o.meaning, o).has(HE.K(CARD)), `${CARD} אינו בקבוצת הבעלים של ${TWIN} · הפטור אינו סימטרי`);
    /* 2 · "המילה הלא נכונה" אינה מה שקורה כאן · אותה מחרוזת מתקבלת על **שניהם**,
       וזה בדיוק מה שפירוש משותף אומר. אילו התקבלה על אחד בלבד, זו הייתה התנגשות. */
    for (const typed of REPORTED.concat([CLEAN])) {
      assert.strictEqual(HE.meaningMatch(typed, w.meaning, w), true, `${CARD} דחה "${typed}"`);
      assert.strictEqual(HE.meaningMatch(typed, o.meaning, o), true, `${TWIN} דחה "${typed}"`);
    }
    /* 3 · השיניים · אותה קריאה בדיוק, בלי הנרדף בקבוצת הבעלים. זהו המצב שייווצר אם
       יוחלף הפירוש של אחד מהשניים בלבד. שלוש המחרוזות נפסלות כהתנגשות, **וביניהן
       המאויתת נכון** · ולכן הקבלה של היום אינה תלויה בשכבת השגיאות. */
    const alone = new Set([HE.K(CARD)]);
    for (const typed of REPORTED.concat([CLEAN])) {
      const v = HE.nearMatch(HE.norm(typed), segs, 'he', HE.TYPO_PARAMS.gloss, HE.SEG_VETO, alone);
      assert.strictEqual(v.ok, false, `"${typed}" התקבל על ${CARD} גם בלי הפטור · שומר ההתנגשות אינו נוגע במקרה הזה`);
      assert.strictEqual(v.why, 'collision', `"${typed}" נפסל מסיבה אחרת (${v.why}) · הבדיקה אינה מודדת את שומר ההתנגשות`);
    }
  });
  ...
  test('שכבת הלקסיקון נושאת במשקל · בלעדיה נפתחות בדיוק הקבלות שנמדדו', t => {
    ...
    /* המספרים נמדדו על אותה טבלת זהב. הם מוצמדים ולא מושווים ל"גדול מאפס":
       ...
       ‏en-word עלה 12 → 21 עם השוליים המדורגים, ו-he-word 8 → 10 כשהגיע תורה:
       הסובלנות התרחבה, ולכן היו יותר מחרוזות שרק הלקסיקון עוצר.
       ואז `W.sub` הועלה מ-1 ל-2 (הכרעת חגי), והמספרים ירדו ל-8 ול-20.
       ⚠ **ירידה כאן היא בדרך כלל הסימן המדאיג** — היא אומרת שהשכבה נחלשה
       בשקט. כאן היא לגיטימית ומוסברת: הידקנו את הבודק בכוונה, ולכן חלק
       מהמחרוזות שהלקסיקון היה עוצר כבר לא מגיעות אליו — הן נופלות קודם על
       המרחק. ההבחנה בין השתיים היא בדיוק למה המספר מוצמד ולא מושווה
       ל"גדול מאפס": כל תזוזה כאן מחייבת הסבר.
       ‏en-word עלה 20 → 22 עם מקדמי התכונה (aFirstTight/aShareTight). זו
       **העלייה** שהפסקה למעלה מתארת כלגיטימית: המשטר הצר החליף איסור גס
       (‏WTight.sub=99) בקנס רציף, ולכן מחרוזות שקודם נפלו על המרחק מגיעות
       עכשיו עד הלקסיקון והוא זה שעוצר אותן. ‏he-word ו-gloss לא זזו, כצפוי,
       כי הפרמטרים שלהם ביט-זהים למה שנשלח אתמול.
       ‏en-word עלה 22 → 23 עם 445 תיקוני הפירוש מהביקורת העיוורת. ⚠ **הפעם
       הפרמטרים לא זזו כלל — הנתונים זזו.** ‏445 פירושים קיבלו מובן נוסף, ולכן
       מחרוזות עבריות שקודם לא היו בבעלות אף כרטיס נעשו תפוסות, והלקסיקון הוא
       זה שעוצר עוד אחת מהן. זו העלייה שהפסקה למעלה מתארת כלגיטימית, והכיוון
       הוא הנכון: המספר **עלה**, כלומר השכבה נושאת יותר משקל ולא פחות.
       ‏5.9.2026 · ‏he-word 8 → 11, en-word 23 → 24, gloss 5 → 7. הפרמטרים לא
       זזו — **הדגימה של טבלת הזהב הוחלפה**: המכסה נגזרת עכשיו לכל שפה בנפרד
       (‏evolve.js · GOLDEN_TARGET_BY_LANG) במקום מהסכום המשותף, ולכן השכבות
       והזרעים השתנו והטבלה נדגמה מחדש. מכאן והלאה גידול במאגר העברי אינו נוגע
       במספרים האנגליים כאן — הם זזים רק כשהצד האנגלי עצמו זז, וזה בדיוק מה
       שההצמדה אמורה לתפוס. */
    assert.deepStrictEqual(opened, { 'he-word': 11, 'en-word': 24, gloss: 7 },
...
describe('סובלנות איות · טסט הסיום של שתי המילים', () => {
  ...
  /* המשקל שמתמחר אות כפולה. שתי הקבלות למטה עוברות דרכו ודרכו בלבד, ולכן הן
     ...
     ⚠ עם השוליים המדורגים יש **שני** משקלים כאלה, ושתי המילים האלה מוכרעות
     במשטר הצר · שתיהן חיות בשכונה צפופה (ל-אָמִיר שני תאומים במרחק 1, ולכתיב
     המלא של מִכְמוֹרֶת יש שכנים), ולכן ה-gap שלהן מתחת ל-marginSoft. לכן המחיר
     הצפוי הוא של המשטר הצר. אם זה יתהפך למשקל הראשי — השכונה השתנתה, וזה בדיוק
     מה שכדאי לדעת; הבדיקה תיפול ותגיד את זה. */
  const GRADED_HE = () => {
    const p = HE.TYPO_PARAMS['he-word'];
    return p.marginSoft != null && p.marginHard != null && p.marginSoft > p.marginHard;
  };
  const DOUBLE = () => {
    const p = HE.TYPO_PARAMS['he-word'];
    return GRADED_HE() && p.WTight ? p.WTight.doubleLetter : p.W.doubleLetter;
  };

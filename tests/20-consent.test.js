...
/* ===================================================================================
 * FIX 3 · שרשרת ההסכמה: הדפים המשפטיים קיימים ומקושרים
 * ===================================================================================
 * "השימוש בשירות מהווה הסכמה לתנאים" מחזיק רק אם המשתמש יכול להגיע לתנאים. ס' 11
 * לחוק הגנת הפרטיות דורש שהיידוע יימסר בנקודת איסוף המידע — טופס ההרשמה — ולא בדף
 * יתום. הסיכון שהקבוצה חוסמת הוא בדיוק הכשל הרב-סשני שכבר קרה עם CSS הגלולה: סשן
 * שנוגע ב-index.html מוחק את הקישורים בלי לדעת שהדפים קיימים, וכל צד תקין לבדו. */
describe('FIX 3 · הדפים המשפטיים קיימים ומקושרים מנקודת האיסוף', () => {
  ...
  test('טופס ההרשמה נושא את היידוע: קישור לתנאים ולמדיניות', () => {
    const legal = html.match(/<p class="au-legal">[\s\S]*?<\/p>/);
    assert.ok(legal, 'שורת au-legal נעלמה ממסך ההרשמה — היידוע כבר לא בנקודת האיסוף');
    assert.ok(legal[0].includes('href="terms.html"'), 'ההרשמה לא מקשרת לתנאי השימוש');
  ...
  test('כל דף מקשר לשני האחרים, נושא כתובת קשר, ונקי ממקף ארוך', () => {
    for (const p of pages) {
      const t = fs.readFileSync(path.join(ROOT, p), 'utf8');
      for (const q of pages.filter(x => x !== p))
        assert.ok(t.includes(`href="${q}"`), `${p} לא מקשר אל ${q}`);
      assert.ok(t.includes('admin@800-plus.com'), `${p} בלי כתובת קשר`);
      /* המקף הארוך אסור בכל טקסט יוצא (HEB §3א) — בטקסט הגלוי, לא בהערות קוד. */
      assert.ok(!/—/.test(t.replace(/<!--[\s\S]*?-->/g, '')), `${p} מכיל מקף ארוך`);
    }
  });
});

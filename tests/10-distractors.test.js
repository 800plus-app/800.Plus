...
describe('feedback timers — nothing may outlive the exit button', () => {
  ...
  test('the level-test answer timer is stored so #lvExit can cancel it', () => {
    ...
    assert.ok(pick.length >= 1, 'no deck-advancing timer assigns to lvTimer at all any more');
   ...
   *
   * Written as a count of ZERO rather than a count of one at a known line: a rule that says
   * "every deck-advancing timer is cancellable" also covers the next one somebody adds, which
   * a line number never would. Replaced the pinned version per its own instructions. */

...
describe('flushRemoteSync · the round trip the app actually runs', () => {
   ...
   * ones tests the same thing and can no longer be confused with an orphan. */
  test('a cloud row is merged before the write, not overwritten by it', async () => {
    let ref = null;
    const s = loadSyncLayer({
      respond: {
        'progress.select': () => ({ data: { data: P({
          stats: { words: { [ref.K(ref.BANK[1].term)]: R({ seen: 7, last: 99 }) }, sessions: [] } }) } }),
        'progress.upsert': {},
      },
    });
    ref = s.ctx;
    const localWord = s.ctx.K(s.ctx.BANK[0].term), remoteWord = s.ctx.K(s.ctx.BANK[1].term);
    assert.notStrictEqual(localWord, remoteWord, 'sanity: the two sides must hold different words');
    s.ctx.stats = { words: { [localWord]: s.ctx.saneRec(R({ seen: 1, last: 5 })) }, sessions: [] };
  ...
  test('a failed write leaves the save queued, so the next flush tries again', async () => {
    const s = loadSyncLayer({
...
describe('identity · the account is resolved twice per sync, independently', () => {
  ...
  test('BUG: an account that changes between the read and the write sends A\'s data into B\'s row', async () => {
    ...
    const s = loadSyncLayer({
      ...
      respond: {
        'progress.select': () => ({ data: { data: P({
          stats: { words: { [ref.K(ref.BANK[0].term)]: R({ seen: 5, last: 3 }) }, sessions: [] } }) } }),
        'progress.upsert': {},
      },
    });
    ref = s.ctx;

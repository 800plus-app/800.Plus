...
describe('t0 · מתי מילה נחשבת "נלמדה"', () => {
  ...
  test('⭐ ידע מיד ואז שכח ⇒ נכנס ליחידה באותו רגע', () => {
    /* ⛔ הפער שנשאר אחרי שהשחזור תוקן. `backfillT0` מסמן כל מילה עם `wrong>0`,
       בדיוק כמו שמסך הסטטיסטיקה סופר אותה תחת "המילים שטעית בהן" · אבל הכתיבה
       החיה ב-commitSession דרשה רשומה **בתולית** (first+ever+wrong===0). מילה
       שהלומד ידע בניסיון הראשון ואז שכח קיבלה `wrong=1` ולא קיבלה `t0`.
       התוצאה שהלומד ראה: המסך סופר אותה כמילה שטעה בה, והיחידה אינה מכילה אותה
       עד הטעינה הבאה, שבה השחזור מוסיף אותה בשקט. אותה מילה, שתי תשובות, לפי
       מתי במקרה נסגרה האפליקציה.
       ⚠ הבדיקה אינה על סדר ואינה על תאריך · היא על **חברות ביחידה**. */
    const ctx = fresh();
    const w = ctx.uniqScope('unit:1')[0];
    practiseRound(ctx, [[w, 'first']], { scope: 'unit:1' });
    assert.strictEqual(t0of(ctx, w), undefined, 'הבקרה: אחרי "ידע מיד" עדיין אין t0');

    practiseRound(ctx, [[w, 'wrong']], { scope: 'unit:1' });
    assert.strictEqual(ctx.stats.words[ctx.K(w.term)].wrong, 1,
      'הבקרה: המסך סופר אותה תחת "המילים שטעית בהן" · `wrong` הוא האות שלו');
    assert.ok(t0of(ctx, w) > 0,
      'מילה שידע ואז טעה בה לא קיבלה t0 · המסך סופר אותה והיחידה מפספסת אותה');
    assert.ok(ctx.acquiredCards('unit:1').some(x => ctx.K(x.term) === ctx.K(w.term)),
      'המילה אינה ביחידת החזרות אף שהמסך סופר אותה כמילה שטעית בה');
  });

  test('⛔ הכתיבה החיה והשחזור מסכימים · אותו כלל, לא שניים', () => {
    /* שני מסלולים כותבים `t0`: `commitSession` בזמן אמת ו-`backfillT0` בטעינה.
       כשהם אינם מסכימים, הרשימה משתנה אחרי רענון בלי שהלומד עשה דבר. כאן נמדדת
       ההסכמה עצמה: אחרי הסבב, שחזור על אותו מצב אינו מוסיף אף מילה. */
    const ctx = fresh();
    const ws = ctx.uniqScope('unit:1').slice(0, 3);
    practiseRound(ctx, ws.map(w => [w, 'first']), { scope: 'unit:1' });
    practiseRound(ctx, [[ws[0], 'wrong'], [ws[1], 'struggle'], [ws[2], 'first']], { scope: 'unit:1' });
    const before = ctx.acquiredCards('unit:1').length;
    assert.strictEqual(ctx.backfillT0(ctx.stats.words), 0,
      'השחזור מצא מילים שהכתיבה החיה החמיצה · היחידה תגדל מעצמה ברענון הבא');
    assert.strictEqual(ctx.acquiredCards('unit:1').length, before);
    assert.strictEqual(before, 2, 'הבקרה: שתי המילים שטעה בהן ביחידה, וזו שידע אינה');
  });

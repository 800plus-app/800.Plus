...
describe('הדרישה מוצגת איפה שהיא נחוצה', () => {
  ...
  test('ההסבר המלא מוצג פעם אחת בלבד', () => {
    ...
    assert.match(src, /LS\.get\('hw_sense_intro', false\)/, 'אין דגל לפעם הראשונה');
    assert.match(src, /LS\.set\('hw_sense_intro', true\)/, 'הדגל אינו נשמר — ההסבר יחזור בכל כרטיס');
  });

  test('הניסוח נכון לשני המצבים', () => {
    /* "נדרש פירוש 1" אינו עברית, ולכן שתי שורות ולא תבנית עם מספר. */
    const at = app.indexOf('function senseNeedBlock');
    const src = app.slice(at, at + 900);
    assert.match(src, /עוד פירוש אחד והמילה תיחשב נלמדה/, 'חסר הניסוח למי שנתן פירוש אחד');
    assert.match(src, /נדרשים שני פירושים כדי שהמילה תיחשב נלמדה/, 'חסר הניסוח למי שלא נתן כלום');

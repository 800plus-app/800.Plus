...
/* ⚠ אותיות סופיות. בלי הנרמול הזה `אנזים` אינו נפגש עם `אנזימים`: ם ומ הם שני תווים
   שונים, ולכן כל שם עצם ברבים או בנטייה נראה כמו מילה שאינה בתרגום. נמצא ב-12.8
   כשתרגום שהשתנה חזר לסימון אוטומטי ונפל — עד אז הפריטים האלה פשוט הגיעו לטיפול
   ידני, ולכן הכשל היה שקט: הוא לא ייצר סימון שגוי, הוא ייצר עבודה מיותרת. */
const fin = w => w.replace(/ך/g, 'כ').replace(/ם/g, 'מ').replace(/ן/g, 'נ')
...
const bare = w => { const b = fin(w).replace(/[אוי]/g, ''); return b.length >= 3 ? b : null; };
...
function findSpan(gl, he) {
  ...
  /* סדר העדפה: **סדר המשמעות בבנק קודם לכול** (הראשית ראשונה), אחר כך דרגת
     ההתאמה, ורק בתוך אותה משמעות מנצח הצירוף השלם על מילה בודדת מתוכו.
     ⚠ עד 12.8 מספר המילים היה ראשון, והצירוף ניצח תמיד. נמדד שזה מדגיש יותר מדי:
     ב-`damage` הפירוש הוא `נזק, לגרום נזק`, והמשפט `הסופה גרמה נזק חמור` סומן
     `גרמה נזק` — כלומר גם `caused`, שאינה המילה הנלמדת, הודגשה. אותו דבר ב-`list`
     (`הכנתי רשימת` במקום `רשימת`).
     ⭐ ומדוע לא פשוט "המקטע הקצר תמיד": ב-`nutrient` הפירוש היחיד הוא `חומר מזין`,
     וסימון `חומרים` לבדו קוטע מונח. סדר המשמעות מפריד בין שני המקרים בלי לנחש:
     כשיש משמעות ראשית קצרה שנמצאת במשפט, היא הנכונה. */
  /* ⛔ ניסיתי להעביר את סדר המשמעות לראש המפתח, וזה **הוחזר**. נמדד: 64 פריטים
     השתנו בבת אחת והכיסוי ירד, מפני שהמפתח מתערבב עם כלל אי-ההכרעה — מועמד ממשמעות
     אחרת במקום אחר במשפט הופך את ההתאמה ל"לא מוכרעת" ומבטל אותה. שינוי במפתח הדירוג
     נוגע בכל 3,946 הפריטים בו-זמנית, כלומר הוא מפקיע את הביקורת שכבר נעשתה עליהם.
     ההכרעה: המפתח נשאר, ושני הפריטים שנמצאו בעייתיים (`damage`, `list`) מסומנים
     ידנית. סימון ידני גובר על האוטומטי, ולכן התיקון מדויק ואינו גורף. */

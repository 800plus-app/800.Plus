...
/* ⭐ גרש וגרשיים **בין שתי אותיות עבריות** הם חלק מהמילה ואינם פיסוק ·
   `תנ"כי` · `ג'ונגל` · `צה"ל`. הקילוף העיוור פיצל אותם לשתי מילים, ולכן
   `בחו"ל` נקרא `בחו ל` ו-`חו"ל` נקרא `חו ל`.
   ⛔ שאר הפיסוק ממשיך לרדת, וגם מרכאה בקצה מילה · השומר חל **רק** כשיש
   אות עברית משני הצדדים. הניקוד הוסר בשלב שלפני, ולכן השכנים הם אותיות ממש. */
const HEB_LETTER = /[א-ת]/;
const QUOTE_CH = /["'`״׳“”‘’]/;
function stripQuoteAware(m, i, str) {
  if (QUOTE_CH.test(m) && HEB_LETTER.test(str[i - 1] || '') && HEB_LETTER.test(str[i + 1] || ''))
    return m;
  return ' ';
}
...
function selftest() {
  ...
  /* ⭐ גרש וגרשיים בתוך מילה עברית · נשארים */
  T('canon · גרשיים בתוך מילה', canon('תנ"כי') === 'תנ"כי');
  T('canon · גרש בתוך מילה', canon("ג'ונגל") === "ג'ונגל");
  T('canon · ראשי תיבות', canon('צה"ל') === 'צה"ל');
  T('canon · בחו"ל מול חו"ל', canon('בחו"ל') === 'בחו"ל' && canon('חו"ל') === 'חו"ל');
  /* ⛔ ואלה **חייבים** להמשיך לרדת · אחרת חסמנו הכול במקום להחמיר */
  T('canon · מרכאה בקצה יורדת', canon('"שלום"') === 'שלום');
  T('canon · מרכאה מול לטינית יורדת', canon('it\'s') === 'it s');
  T('canon · מרכאה מול רווח יורדת', canon('שלום " עולם') === 'שלום עולם');
  T('canon · שאר הפיסוק יורד', canon('א.ב,ג;ד') === 'א ב ג ד');

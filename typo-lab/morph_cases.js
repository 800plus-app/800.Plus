...
function measureMulti(key, lang, opts) {
  ...
  const res = {
     ...
     *           המקורי (`דוחות/מדידת-כלל-מורפולוגי.md §א`) נקב בשמו. */
...
function selftest() {
  ...
  out.push('ד · מנוע רב-המילים מוצא התנגשות שידועה מראש');
  {
    /* מִצְנֶפֶת "כובע בד, מגבעת" · הקלדת "כובע" היא הדוגמה שהדוח המקורי נקב בשמה
       (דוחות/מדידת-כלל-מורפולוגי.md §א): היא תשובה קבילה גם של מִגְבַּעַת ושל
...
function report() {
  ...
  p('');
  p('זו בדיוק ההתנגשות ש-`דוחות/מדידת-כלל-מורפולוגי.md §א` נקב בשמה. לכן הטבלה סופרת');

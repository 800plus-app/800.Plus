 ...
 */
/* ═══════════════════════════════════════════════════════════════════════════
 * ⭐⛔ **‏T5 אינה מצביעה בכיוון `word`** · נבחר על `en-blind3` · 19.8.2026
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * ההערה שמעל תיארה את העיקרון מאז ומתמיד — *"בכיוון `word` שם T5 מצביעה
 * **הפוך**"* — והוא פשוט לא נאכף אחרי שהוסרה `decideWordDir`. עכשיו הוא נאכף,
 * ו**לא כי הוא נשמע נכון אלא כי הוא נמדד מחוץ למדגם**.
 *
 * ⛔ **ההבדל היחיד שמכשיר את זה, אחרי ש-`decideWordDir` נפלה:** ההכרזה קדמה
 * למדידה. הקריטריון נכתב לקובץ ונכנס לקומיט `fabff15` **לפני** שהורצה שורה
 * אחת על `en-blind3` ולפני שהיה לו פנקס · `typo-lab/blind3_verdict.js`.
 * ‏`decideWordDir` נבחרה אחרי שנראו הסטים; זה לא.
 *
 * הפסק · הרצה **אחת** · 16 שורות `word`, כל אחת מתויגת:
 *
 *   | | דיוק | recall | קבלות-שווא |
 *   |---|---|---|---|
 *   | החוק הקודם | 62.5% | **0.0%** | 0 |
 *   | היום        | **100.0%** | **100.0%** | **0** |
 *
 * ⭐⭐ **ולמה זה **לא** חוזר על כשל `decideWordDir`** — זו כל השאלה, והסט נבנה
 * במיוחד כדי לענות עליה. הוא מכיל את שתי המחלקות שנראות זהות ל-T5, שש מכל אחת:
 *
 *   | המחלקה | ‏T5 | האמת | מי דוחה היום |
 *   |---|---|---|---|
 *   | «אינה מילה · קרובה» `bridge`→`briddge` | `ל` | **כ** | אף אחת · מתקבל ✅ |
 *   | «זבל רחוק» `favored`→`cvjzhze` | `ל` | **ל** | ⭐ **T3** |
 *   | «מילה אחרת» `drug`→`rule` | `כ` | **ל** | ‏T2 ו-T3 |
 *
 * ‏`decideWordDir` נפלה כי היא החזירה `accept` בענף `if (notWord)` **בלי
 * להתייעץ באף עדשה**, ולכן בלעה את «זבל רחוק» יחד עם «קרובה». כאן T5 רק
 * **אינה מצביעה**; כל `ל` של T2 או T3 עדיין דוחה, ו**T3 היא שתופסת את הזבל**
 * — ‏6 מתוך 6, נמדד. ‏0 קבלות-שווא בשלוש המחלקות.
 *
 * ⚠ **המכסה יורדת ל-2 בכיוון `word`, וזה חלק מהחוק ולא תוספת.** נמדד: הוצאת
 * T5 בלי הורדת המכסה היא **no-op מוחלט** — בכיוון `word` נשארות T2 ו-T3 בלבד,
 * ומכסת 3 מפילה כל שורה ל-`unsure`. חוק שמוציא עדשה חייב להוריד את המכסה,
 * אחרת הוא השבתה ולא חוק.
 *
 * ⚠⚠ **ומה שזה עדיין אינו:** ‏n=16. הפסק מסיר את הפסילה המתודולוגית שרבצה על
 * המספרים בתוך-המדגם (‏`en40` · `en2`), והוא **אינו** הופך את החוק למוכח.
 * ‏T5 נשארת בפנקס, נשארת מצביעה ב-`gloss`, ופסקיה ב-`word` נשמרים כמדד
 * ("כמה מהמתקבלות אינן מילה כלל"). היא לא הושבתה — היא הוצאה מהצבעה אחת. */
function decide(it, v) {
  ...
  const wordDir = it.direction === 'word';
  const app = applicable(it).filter(l => !(wordDir && l === 'T5'));
  const got = app.filter(l => v[l]);
  if (app.some(l => v[l] === 'ל')) return 'reject';          /* ⛔ כל "ל" עדיין דוחה */
  if (got.length < (wordDir ? 2 : 3)) return 'unsure';       /* היעדר פסק אינו קבלה */
...
function selftest() {
  ...
  t(decide(en('abacus', 'abavus'), { T5: 'ל' }) === 'unsure', '⛔ פסק של T5 לבדה ⇒ unsure · היא אינה מצביעה בכיוון word, ומתחת למכסה');
  /* ═══ ⭐⛔ **החולשה שתועדה ונפתרה · `en-blind3` · 19.8.2026** ═══
     כאן עמדה השן ההפוכה, עם ההערה *"זו החולשה שהסט השלישי אמור לכמת"*.
     ‏`en-blind3` הגיע, נמדד **פעם אחת** לפי קריטריון שהוכרז בקומיט `fabff15`
     לפני שנגענו בו, ועבר: ‏16/16 · דיוק 100% · recall ‏0%→100% · אפס
     קבלות-שווא. ראה `typo-lab/blind3_verdict.js`. */
  t(decide(en('abacus', 'abavus'), { T2: 'כ', T3: 'כ', T5: 'ל' }) === 'accept',
    '⭐ שגיאת הקלדה שאינה מילה ⇒ **accept** · T5 אינה מצביעה בכיוון word · הפסק של en-blind3');
  t(decide(en('abacus', 'abavus'), { T2: 'כ', T3: 'כ' }) === 'accept', 'שתי עדשות בכיוון word ⇒ מכסה מלאה · T5 אינה חלה שם ואינה "חסרה"');
  t(decide(en('abacus', 'abavus'), { T2: 'כ' }) === 'unsure', '⛔ עדשה אחת ⇒ unsure · המכסה ירדה ל-2, לא ל-1');
  /* ⭐⛔ **השן שמונעת את החזרה של `decideWordDir`** · שלוש המחלקות מ-`en-blind3` */
  t(decide(en('favored', 'cvjzhze'), { T2: 'כ', T3: 'ל', T5: 'ל' }) === 'reject',
    '⭐⛔ «זבל רחוק» ⇒ **reject** · ‏T3 תופסת אותו, לא T5 · זו הסיבה שהוצאת T5 בטוחה');
  t(decide(en('drug', 'rule'), { T2: 'ל', T3: 'ל', T5: 'כ' }) === 'reject',
    '⛔ מילה אנגלית אחרת ⇒ reject · ‏T2 ו-T3 · ו-T5 דווקא אמרה "כ"');
  t(decide(en('notwithstanding', 'awfagivlffawwvs'), { T2: 'ל', T3: 'ל', T5: 'ל' }) === 'reject',
    '⛔ X20 · שלוש "ל" ⇒ reject גם אחרי שהוצאנו את T5 · ‏R2 אינו עוקף את הפאנל');
  /* ⛔ כיוון `gloss` לא נגוע · שם T5 כן מצביעה והמכסה נשארת 3 */
  const g0 = itemOf({ lang: 'he', direction: 'gloss', term: 'עקר', gloss: 'חיטא', written: 'הרג חיידקימ', typed: 'הורג חיידקימ' });
  t(decide(g0, { T2: 'כ', T3: 'כ' }) === 'unsure', '⛔ ‏gloss · שתי עדשות ⇒ unsure · המכסה שם נשארה 3');
  t(decide(g0, { T2: 'כ', T3: 'כ', T5: 'ל' }) === 'reject', '⛔ ‏gloss · "ל" של T5 עדיין דוחה · היא לא הושבתה, רק אינה מצביעה ב-word');

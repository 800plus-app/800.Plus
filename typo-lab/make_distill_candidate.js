...
function main() {
   ...
   *
   * ⛔⛔ **תיקון · נוסח קודם כאן היה שגוי, וזו טעות שלי.** נכתב כאן ש-`useLexicon:false`
   * "אינו מסיים בזמן סביר", על סמך ריצה שנזנחה. ⚠ **בדיקת השעון אחר כך הראתה
   * שהיא רצה ‏~8 דקות בלבד**, בזמן שריצת שער תקינה לוקחת **~20 דקות** ומדפיסה את
   * שורות השכבה רק לקראת הסוף (הבנייה של 10.2M המועמדים קודמת לכל פלט). כלומר
   * ‏"‏8 דקות בלי שורת שכבה" הוא **בדיוק מה שריצה בריאה נראית בשלב הזה**, ולא עדות
   * לכלום. בנוסף רצה באותו זמן תהליך כפול שהתחרה על ה-CPU.
   * ⭐ **הטענה נמשכת. אין לי שום מדידה על מה שקורה כשמכבים את הווטו** — לא על
   * זמן הריצה ולא על ההתנגשויות. מי שירצה לתמחר את 37 הקבלות צריך להריץ את זה
   * מחדש ולתת לו לסיים.
   *
   * ⚠ הלקח נרשם כי הוא הדפוס שהפרויקט הזה מתעד שוב ושוב: **הצגתי השערה
   * ("היקום התפוצץ") כעובדה מדודה**, ובנוסח כזה היא הייתה נכנסת ל-STATE.md
   * ומונעת מהסוכן הבא לנסות. אומדן זמן שלא נמדד מול שעון אינו נתון.
   *

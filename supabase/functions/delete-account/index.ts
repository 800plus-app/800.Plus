 ...
 *
 * verify_jwt מוגדר false, וההנמקה המלאה נמצאת ב-supabase/config.toml ליד
 * [functions.delete-account]. אין כאן פשרה: הפונקציה קוראת ל-auth.getUser(token)
 * בעצמה, וזו בדיקה חזקה יותר מהשער הגנרי — בלי טוקן תקף היא מחזירה 401.
 * (השורה שעמדה כאן אמרה "verify_jwt נשאר true", בסתירה ישירה ל-config.toml.
 *  לא הייתה כאן פגיעות, אבל שתי אמירות הפוכות באותו נושא בשני קבצים סמוכים הן
 *  בדיוק מה שמייצר תיקון שגוי בסבב הבא — ובמאגר ציבורי גם מטעה קורא מבחוץ.
 *  הנימוק חי במקום אחד בלבד, זה שבו ההגדרה עצמה יושבת.)
 *

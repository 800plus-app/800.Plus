...
const PROVIDER_MAP = {
  payplus(evt: any): Norm {
    ...
    return {
      ...
      /* אין נפילה ל-crypto.randomUUID(). האילוץ הייחודי ב-10.sql הוא כל
         האידמפוטנטיות, ו-`10.sql:64-68` מסביר באריכות למה `select ואז insert`
         הוא מרוץ ולמה רק אילוץ מכריע אותו — ואז ערך אקראי הופך את האילוץ הזה
         לחסר משמעות, כי שני משלוחים של אותו אירוע מקבלים שני מזהים שונים
         ושניהם נכנסים. משלוח כפול שהמיפוי לא מזהה נרשם פעמיים.
         `null` כאן נדחה במפורש בהמשך: עדיף לדחות אירוע שאיננו יודעים לזהות
         מאשר לרשום אותו פעמיים ולגלות את זה בהתאמת ספרים. */
      event_id: ((v) => (v == null ? null : String(v)))(
        pick(rec, 'charge_uid')
...
Deno.serve(async (req) => {
  ...
  /* אירוע שאין בו מזהה שאנחנו יודעים לקרוא נדחה, ובקול.
     400 גורם לספק לנסות שוב ואז להתריע — וזה בדיוק מה שרוצים: אם המיפוי
     שגוי, עדיף שכל אירוע ייפול ברעש מאשר שכל אירוע יירשם פעמיים בשקט
     ויתגלה רק בהתאמת ספרים.
     ⚠ לפני go-live: לשלוח מטען אמיתי אחד מהספק ולוודא שהוא עובר כאן. אם
     המיפוי לא מכיר את שם השדה של PayPlus, זה הקו שיעצור את כל החיובים. */
  if (!n.event_id) {
    console.error('אירוע בלי מזהה שניתן לזיהוי — נדחה', JSON.stringify(evt).slice(0, 300));
    return new Response('unidentifiable event: no known id field', { status: 400 });
  }
  ...
  /* גם `false` וגם `null` אינם מעניקים גישה, ורק `true` כן.
   *
   * קודם רק `false` נדחה, ו-`null` נפל דרך התנאי והאירוע **הוחל**. זה נראה
   * כמו הבדל דק והוא לא: `null` מוחזר גם כשמשתני הסביבה של PayPlus לא
   * הוגדרו, וגם — וזה העיקר — **כשאין `recurring_uid` במטען בכלל**. כלומר
   * הדרך הקלה ביותר לעקוף את האימות החוזר הייתה פשוט לא לשלוח את השדה
   * שמפעיל אותו.
   *
   * הקובץ הזה כותב במפורש (שורה 56-57) ש"ה-webhook הוא רמז להסתכל, לא מקור
   * אמת", וכשאין חתימה — במצב `secret_url`, שהוא ברירת המחדל — האימות החוזר
   * הוא **כל** ההגנה. הגנה שמדלגת על עצמה כשחסר שדה אינה הגנה.
   *
   * המחיר של החמרה: תקלת רשת רגעית מול הספק מסמנת חיוב אמיתי לבדיקה במקום
   * להעניק גישה מיד. זה המחיר הנכון — לקוח שממתין דקה נפתר בפנייה אחת,
   * ואילו גישה שניתנה על אירוע מזויף מתגלה רק בהתאמת ספרים. */
  if (confirmed !== true) {
    console.error('החיוב לא אושר ע"י הספק', confirmed === false ? 'נדחה' : 'לא ידוע',
                  n.event_id, n.subscription_id);
    // נרשם בספר ולא מוחל: סטטוס null אומר "אל תיתן גישה", והאירוע מסומן
    // לבדיקה. לא מחזירים 400 — ה-webhook עצמו היה תקין.
    n.status = null;
  }

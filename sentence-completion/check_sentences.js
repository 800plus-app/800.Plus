/* שער האיכות להשלמת משפטים · דוחות/רעיונות/השלמת-משפטים-אנגלית-תוכנית.md §6.3
 *
...
for (const [level, items] of Object.entries(SENT)) {
  ...
  items.forEach((it, i) => {
    ...
    /* ⚠ `e` הוא שדה הפורמט **הישן**: פסקה אחת שנכתבה לפני שידענו מה הלומד בחר.
       הפורמט שאושר ב-10.8 מפצל אותו ל-`g`/`t`/`r`, ו-`r` הוא שמוצג ללומד. ב-204
       הפריטים הראשונים `e` נשמר כארכיון וכמקור שממנו `r` נגזר, אבל פריט **חדש**
       נכתב ישר בפורמט החדש — וחיוב `e` היה מחייב כתיבה כפולה של אותו תוכן.
       הדרישה היא אחד מהשניים: `e` (פריט מהסבב הראשון) או `r` מלא (פריט חדש). */
    const rFull = Array.isArray(it.r) && it.r.length === it.o.length && it.r.every(x => x && String(x).trim());
    if ((!it.e || !it.e.trim()) && !rFull)
      E(id, 'אין הסבר: נדרש `e` (פורמט ישן) או `r` מלא לכל האפשרויות (פורמט חדש)');

 ...
 *   שער השילוח  he-word 0 קבלות-שווא · recall 9.71% (רצפה 4.24%)
 *               en-word 0 קבלות-שווא · recall 45.39% (רצפה 41.46%)
 *               gloss   0 קבלות-שווא · recall 0% (רדום בכוונה · ראה למטה)
 *   טביעת אצבע  fp מתחת · נבדקת מול הארטיפקט בכל ריצת בדיקות
 *
 * ✅ שער המאגר ירוק (15.8.2026). `typo-lab/bank_gate.js` על הפרמטרים האלה:
 * **אפס התנגשויות חדשות**, 5,663 ערכים, 2,460,907 זוגות מחושבים. הריצה הקודמת
 * נפלה שם על 138 התנגשויות ("caughtt" על bought, "110th" על "1st - first"), כי
 * האופטימיזציה ניקדה כל וריאציה מול הכרטיס שלה בלבד ולא מול 5,662 האחרים. האילוץ
 * החוצה-כרטיסים נכנס לתוך פונקציית הכושר, ומאז הוא מה שמחזיק את המספר על אפס.
 *
 * ⛔ **צד הפירוש רדום מבנית, ולא מכוילן.** כל ארבע רצועות הסף ב-gloss הן 0, ולכן
 * שום מרחק אינו נכנס לסף ואף התנגשות חדשה אינה אפשרית — מבנית, לא בזכות כיול.
 * הסיבה: השער מצא שתי התנגשויות בצד הזה ("רסיס עצ" על אֵגֶל, בבעלות שבב ·
 ...
 * האילוץ החוצה-כרטיסים — שתי השכבות שאמורות לתפוס בדיוק את זה. המחיר 7.06%
 * recall בצד הפירוש, והדרך להחזיר אותו היא להזרים את צורות ההרחבה של B1 לתוך
 * בנאי האילוץ החוצה-כרטיסים, לא להרפות סף.
...
const TYPO_PARAMS = {
  ...
  fp: '3d7d3419',
  'he-word': { dir:'word', minLen:6, vetoMargin:1, useLexicon:true,
    bands:[{maxLen:3,t:0},{maxLen:7,t:0.5164},{maxLen:14,t:0},{maxLen:null,t:1.5253}],
    W:{sub:1,adjSub:1.5771,transpose:1.9474,ins:1.6279,del:0.5165,doubleLetter:0.4423,materVI:0.9335,homophone:1.7535} },
  'en-word': { dir:'word', minLen:1, vetoMargin:2, useLexicon:true,
    bands:[{maxLen:2,t:0.0889},{maxLen:9,t:1.5657},{maxLen:10,t:1.055},{maxLen:null,t:1.4513}],
    W:{sub:1,adjSub:1.3764,transpose:1.0148,ins:1.4561,del:1.534,doubleLetter:1.0232,materVI:0.596,homophone:1.6427} },
  /* כל ארבע הרצועות 0 · הרדימה המבנית שההערה למעלה מסבירה. אין כאן סף להרפות. */
  gloss: { dir:'gloss', minLen:4, vetoMargin:2, useLexicon:true,
    bands:[{maxLen:4,t:0},{maxLen:9,t:0},{maxLen:13,t:0},{maxLen:null,t:0}],
    W:{sub:1,adjSub:2,transpose:2.8453,ins:1.9206,del:0.2,doubleLetter:0.2,materVI:0.9063,homophone:0.2} },
};

...
const PAY_FROM='2026-10-01';

/* ⛔ חגי, 30.8.2026: «תוריד אותה» · תחילה על מסך הכניסה למי שאינו רשום, ואז
   «שים לב שזה גם באתר כשאתה רשום» ⟵ להוריד בשני המקומות.
   ⭐ למה מתג ולא מחיקה: התאריך עצמו נדחה (PAY_FROM 1.10) והפס אמור לחזור כשהמעבר
   לתשלום ייקבע סופית. מחיקה הייתה מחייבת לכתוב מחדש גם את כלל הניסוח העברי
   («1 ימים» · «יומיים» · «היום») שכבר נבדק ועובד.
   ⚠ להחזיר = להחליף ל-true. הגידור על `currentUser` נשאר גם אז, לפי הבקשה
   הראשונה: מי שאינו רשום אינו בתקופה החינמית ואינו יכול לאבד אותה. */
const PAY_BAR_ON = false;
...
function renderPayCountdown(){
  ...
  /* ⛔ חגי, 30.8.2026, עם צילום של מסך הכניסה: «בדף הכניסה למי שלא רשום יש את
     ...
     ולא רק בטעינה הבאה. */
  if(!PAY_BAR_ON || !currentUser){ bar.classList.add('hidden'); bar.innerHTML=''; return; }

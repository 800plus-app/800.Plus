...
const ALT_FILES = [
  /* ⭐ ראשון בעדיפות · ארבע שורות שהוכחו ב-29.8 בשני מנועים שנבנו לשם כך:
   * `lexicon-probe.js` (BDB 1906 ויסטרוב 1903) ו-`ancient-sequence.js`
...
function apply() {
  ...
  for (const p of m.all) {
    ...
    if (p.alt) {
      ...
      /* ⭐ שונה ב-29.8: עד כה נכתבה הערה בלבד והעמודה הראשית המשיכה לטעון
       * «נחלת הכלל» על מהדורה מ-1978. ⛔ זו טענה משפטית שאין לה כיסוי, ולכן
       * העמודה עוברת ל-«דרוש מקור». ⭐ **שום מידע לא נמחק** — הטענה הקודמת
       * והרפרנס נשמרים במלואם בעמודת ההערה, וההחלפה אינה חוזרת בריצה שנייה. */
      if (!/^⚠ דרוש מקור/.test(c[2])) {
        c[4] = ('⚠ ' + p.cat + ' · «נחלת הכלל» אינו מאושר למהדורה מ-1929 ואילך · הטענה הקודמת: ' +
          c[2] + (c[4] ? ' · ' + c[4] : '')).trim();
        c[2] = '⚠ דרוש מקור';
      }
      flagged++;
...
function selftest() {
  ...
  /* בקרה 2 · סף 1929 · ⭐ **הבקרה מזריקה את השורה ואינה מחפשת אותה בטבלה.**
   * ⛔ הגרסה הקודמת חיפשה שורת ספריא 1929+ קיימת, ולכן ברגע ש-18 השורות
   * האלה תוקנו ב-29.8 היא נפלה עם «אין שורה להזיז» — בקרה שניזונה מהפגם
   * שהיא באה לשמור עליו מתה בדיוק כשהפגם נסגר. */
  const seed = rows.findIndex((r, i) => i > 0 && ALLOWED.test((r[2] || '').trim()));
  const inject = y => {
    const b = rows.map(r => r.slice());
    while (b[seed].length < 4) b[seed].push('');
    b[seed][2] = 'נחלת הכלל / CC0 (לפי ספריא)';
    b[seed][3] = 'Injected Control (Jerusalem, ' + y + ' [he])';
    return measure(b).all.length;
  };
  const n1978 = inject(1978), n1850 = inject(1850);
  const p2 = n1978 === base.all.length + 1 && n1850 === base.all.length;
  console.log((p2 ? '✓ ' : '⛔ ') + 'בקרה 2 · אותה שורה עם מהדורה 1978 ⟵ ' + n1978 +
    ' ועם 1850 ⟵ ' + n1850 + ' · הבסיס ' + base.all.length + ' · הסף 1929 הוא מה שמזיז');
  ok = ok && p2;
